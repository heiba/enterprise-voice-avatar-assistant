...
**The face picker does not appear under the avatar frame.** It shows only with two to four entries in `voiceAgent.faces` (extra entries are ignored with a warning) and `voiceAgent.avatarProvider: tavus`; `GET /api/v1/voice/faces` on the frontend Route returns what the page sees. Names missing from the values come from Tavus when `TAVUS_API_KEY` is in the integrations secret; the picker shows initials, not Tavus thumbnails, since those are full videos.
