...
**The face picker does not appear under the avatar frame.** It shows only with two to four entries in `voiceAgent.faces` (extra entries are ignored with a warning) and `voiceAgent.avatarProvider: tavus`; `GET /api/v1/voice/faces` on the frontend Route returns what the page sees. Names and thumbnails come from Tavus when `TAVUS_API_KEY` is in the integrations secret; without it the declared names are used.
