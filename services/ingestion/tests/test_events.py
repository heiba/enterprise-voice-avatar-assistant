from app.events import CREATED, REMOVED, parse_minio_event
...
def test_records_are_parsed_and_keys_decoded():
    ...
    assert parse_minio_event(event) == [(CREATED, "documents", "policies/leave policy.pdf")]
...
def test_removed_events_are_recognised():
    ...
    assert parse_minio_event(event) == [(REMOVED, "documents", "old.pdf")]


def test_top_level_key_is_a_fallback():
    assert parse_minio_event({"EventName": "s3:ObjectCreated:Put", "Key": "inbox/invoice.pdf"}) == [
        (CREATED, "inbox", "invoice.pdf")
    ]


def test_unrelated_events_are_ignored():
    assert parse_minio_event({"EventName": "s3:ObjectAccessed:Get", "Key": "documents/x.pdf"}) == []
